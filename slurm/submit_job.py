...
Now enhanced to pull defaults from a YAML file (including environment) and allow CLI overrides.
"""
...
def main():
    ...
    if defaults_file.exists():
        ...
        for key, value in defaults.items():
            if isinstance(value, str):
                # Expand any env vars embedded in the YAML (e.g. ${ENV})
    ...
    # For each argument, fetch the default from defaults.yaml if it exists
    parser.add_argument(
        "--job_name",
    ...
    # Pull in your environment from the defaults, if specified
    parser.add_argument(
        "--environment",
        default=defaults.get("environment", None),
        help="Environment name (e.g., conda env or a module) to load in the job."
    )
    ...
    # Add 'environment' to the fill dictionary if your template uses it
    filled_text = fill_template(
        template_text,
        {
            ...
            "mem": args.mem,
            "cpus": args.cpus,
            "environment": args.environment,  # Make sure your template references {{ environment }}
        }
    )
    ...
    if args.submit:
        ...
        print(
            f"SLURM script generated at: {output_path}\n"
